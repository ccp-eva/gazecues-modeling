...
write.csv(mgp_triallevel, "../saves/lifespan_mgp_triallevel.csv", quote = F, row.names = F)
saveRDS(mgp_triallevel, "../saves/lifespan_mgp_triallevel.rds")
mgp_triallevel <- readRDS("../saves/lifespan_mgp_triallevel.rds")
```
...
write.csv(mgp_aggregated, "../saves/lifespan_mgp_aggregated.csv", quote = F, row.names = F)
saveRDS(mgp_aggregated, "../saves/lifespan_mgp_aggregated.rds")
mgp_aggregated <- readRDS("../saves/lifespan_mgp_aggregated.rds")
```
...
ggplot() +
  geom_jitter(
    data = mdata, 
    aes(x = age, y = imprecision), 
    col = "darkgrey", alpha = 0.1
  ) + 
  
  geom_smooth(
    data = pred_combined, 
    aes(x = age, y = Estimate, ymin = Q2.5, ymax = Q97.5, group = model, col = model), 
    stat = "identity", size = 0.75, alpha = 0.15
  ) +

  scale_color_colorblind() + 
  scale_x_continuous(breaks = seq(0, 80, 5)) +
  coord_cartesian(ylim = c(0, 4)) +
  labs(x = "Age in years", y = "Imprecision in target width", col = "Model structure")

ggsave("../figures/supplements_lifespan_modelstructure.png", width = 7, height = 5, scale = 1, bg = "white")
ggsave("../figures/supplements_lifespan_modelstructure.pdf", width = 7, height = 5, scale = 1, bg = "white")
```
