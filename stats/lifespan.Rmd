...
library(ggmagnify) # for plot inset
...
cps <- tibble(
          cp = ibeast$trend$cp[1:ibeast$trend$ncp_mode], 
          lowerCrI = ibeast$trend$cpCI[1:ibeast$trend$ncp_mode,1], 
          upperCrI = ibeast$trend$cpCI[1:ibeast$trend$ncp_mode,2]
       )
...
cps_descr <- mdata %>% 
  ...
  summarize(
    ...
    age_max = max(age, na.rm = T),
    n = n(),
    mean = mean(mean_imprecision, na.rm = T) %>% round(2),
    sd = sd(mean_imprecision, na.rm = T) %>% round(2),
  )

saveRDS(cps_descr, "../saves/lifespan_cps_descr.rds")
```
...
lifespan_plot <- mdata %>%
  ...
  # raw data points
  geom_jitter(data = mdata, aes(x = age, y = mean_imprecision), alpha = 0.5, color = "darkgrey", size = 0.75) + 
  ...
  geom_vline(xintercept = cps$cp, linetype = "dashed", alpha = 0.9, size = 0.5) + 
...
```{r magnify}
lifespan_combined <- lifespan_plot +
  scale_x_continuous(breaks = seq(0, 10, 1)) +
  coord_cartesian(clip = "off") + 
  theme(plot.margin = margin(t = 40, r = 60, l = 10, b = 10)) +
  
  # xmin, xmax, ymin, ymax
  geom_magnify(aes(from = age < 10), to = c(55, 90, 2.5, 4.5), axes = "xy", linewidth = 0.5, proj.linetype = 3, proj = "single") + 
  
  scale_x_continuous(breaks = seq(0, 80, 5))

lifespan_combined

saveRDS(lifespan_combined, "../saves/lifespan_plot.rds")
ggsave("../figures/lifespan_plot.png", width = 7, height = 4)
ggsave("../figures/lifespan_plot.pdf", width = 7, height = 4)
```
