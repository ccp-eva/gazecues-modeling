...
```{r variation}
lifespan_variation_all <- lifespan_data %>% 
  summarise(
...
lifespan_variation_agegroups <- lifespan_data %>% 
  group_by(age_group) %>%
  summarise(
    imprecision_mean = mean(click_dist_balloons, na.rm = T) %>% round(2),
    imprecision_sd = sd(click_dist_balloons, na.rm = T) %>% round(2),
...
lifespan_variation_all
write.csv(lifespan_variation_all, "../saves/lifespan_variation_all.csv", quote = F, row.names = F)
saveRDS(lifespan_variation_all, "../saves/lifespan_variation_all.rds")

lifespan_variation_agegroups
write.csv(lifespan_variation_agegroups, "../saves/lifespan_variation_agegroups.csv", quote = F, row.names = F)
saveRDS(lifespan_variation_agegroups, "../saves/lifespan_variation_agegroups.rds")
```
...
ibeast <- beast.irreg(y = y, time = x, season = "none", 
            ...
            # tcp.minmax = c(0, 5), # let the beast decide on its own how many changepoints
            tseg.min = 10, # half of nr data points for decade in adult sample
...
lifespan_plot <- mdata %>%
  ...
  # model predictions spaghetti lines
  geom_line(aes(y = .epred, group=.draw), alpha = .075, color = "blue") +
  # mean model prediction
  stat_lineribbon(aes(y = .epred), .width = 0, color = "black", fill = "black") +
  
  # change points from beast
...
ggsave("../figures/lifespan_all.png", width = 7, height = 4)
ggsave("../figures/lifespan_all.pdf", width = 7, height = 4)
```

```{r zoomed_plot}
# https://cran.r-project.org/web/packages/tidybayes/vignettes/tidy-brms.html
lifespan_plot_zoom <- mdata %>%
  add_epred_draws(mgaussianprocess, ndraws = 100) %>%
  ggplot(aes(x = age, y = mean_imprecision)) +
...
saveRDS(lifespan_plot_zoom, "../saves/lifespan_plot_zoom.rds")

# ggsave("../figures/lifespan_zoom.png", width = 7, height = 4)
# ggsave("../figures/lifespan_zoom.pdf", width = 7, height = 4)
```
...
ggsave("../figures/lifespan_plot.png", width = 10, height = 3, scale = 1.2)
ggsave("../figures/lifespan_plot.pdf", width = 10, height = 3, scale = 1.2)
```
...
lifespan_cut <- mdata %>%
  ...
  scale_x_cut(breaks = c(18), scales = "free", space = 0.001) +

  # model predictions spaghetti lines
  geom_line(aes(y = .epred, group=.draw), alpha = .075, color = "blue") +
  # mean model prediction
  stat_lineribbon(aes(y = .epred), .width = 0, color = "black", show.legend = F) +
  
  # change points from beast
  geom_vline(xintercept = cps$cp, linetype = "dashed", alpha = 0.9, size = 0.75) +
  
  scale_x_continuous(limits = c(3, 80), breaks = c(3:17, 20, 30, 40, 50, 60, 70, 80)) +
  scale_y_continuous(limits = c(0, 4), breaks = seq(0, 4, 0.5)) +

  labs(x = "Age", y = "Mean imprecision", xdist = "", ydist = "")
...
mdata %>%
  ...
  # for gap in x axis
  facet_grid(~ my_facet, scales = "free_x") +
  theme(strip.text.x = element_blank()) + 
  
  ggh4x::facetted_pos_scales(x = list(
    my_facet == FALSE ~ scale_x_continuous(limits = c(3, 20), breaks = seq(3, 19, 1)),
    my_facet == TRUE ~ scale_x_continuous(limits = c(20, 80), breaks = seq(20, 80, 10))
)) + 

  geom_vline(data = data.frame(xint=cps$cp[1:4], my_facet = FALSE), aes(xintercept = xint), linetype = "dotted") +
  geom_vline(data = data.frame(xint=cps$cp[5:6], my_facet = TRUE), aes(xintercept = xint), linetype = "dotted") +
  
  labs(x = "Age in years", y = "Average imprecision in target width") + 
  
  # get rid of space between facets
