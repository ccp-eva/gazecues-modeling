...
# plotting with Rbeast plot function
plot(ibeast)

plot(ibeast, vars = c('t', 'y')) # plot time vs predicted values
plot(ibeast$time, ibeast$trend$Y, type = 'l') # directly plot output: the fitted trend line
plot(ibeast$time, ibeast$trend$cpOccPr) # directly plot output: trend chgpt occurrence prob

# create imprecision as observed in data and as predicted by cp analysis
est <- tibble(age = x, data = y, model = ibeast$trend$Y)
cps <- ibeast$trend$cp[1:ibeast$trend$ncp_median]

# check how many distinct fitted values
# => here question: 
# I'd assume that we need as many distinct
n_distinct(est$model)

# plot raw data and fitted values from cp analysis
est %>% 
