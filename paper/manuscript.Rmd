...
Children came from ethnically homogeneous, mixed socioeconomic backgrounds with mid to high parental education levels.
...
The TANGO has previously been proven reliable for capturing inter-individual differences in gaze understanding [@prein2022tango].
...
Therefore, we formalized the process of gaze understanding in a computational cognitive model.
...
With our model, we seek to explain how participants solved the TANGO.
Our modeling framework consists of three mutually exclusive models: (1) a gaze model, (2) a random guessing model, and (3) a center bias model.
We gauge which model can best explain our data by conducting a model comparison.

\-\-- explanation based on online experiment logic:

The gaze model mirrors the logic of the TANGO programming code.
...
Now, we also know the eyeball radius and, hence, the eyeball boundary.
We then calculate the point of intersection at which the gaze vector meets the eyeball boundary.
Finally, the agent's pupil moves from the center of the eyeball along the gaze vector to the intersection point.
This way, the agent "looks at" the target.

In the computational gaze model, we assume that participants need to go through these steps in reversed order.
The only information that they have readily available is the boundary of the eyeball and the final location of the agent's pupil (i.e., the intersection point).
Now, we assume that participants estimate the center point of the agent's eyeball and then, estimate a line from the center point to the intersection point: this will serve as the gaze vector.
Participants are then thought to extend this gaze vector until it reaches the ground.
They apply this procedure to both eyes and integrate the information.
This way, they can estimate the final target location.

\-\-- explanation based on pre-reg:

The gaze model assumes that the agent focuses their attention on a single point coordinate and that the agent's left and right eyes are positioned by sampling (independently for each eye) gaze vectors from Normal distributions centered on the unique vector that extends from the center of the agent's eye through the center of its pupil to finally meet the attentional focal point (i.e., the modal behavior is to direct both eyes directly toward the focal point).
The standard deviation of these distributions (equal for each eye) is estimated for each individual participant, with higher values increasing the expected imprecision.
...
The gaze model assumes participants use the agent's gaze as a cue to locate the target.
Participants are assumed to observe the current location of the pupil and estimate the agent's eye center.
These two point estimates are sufficient to calculate a vector that points toward the attentional focus of the agent.
Extending this gaze vector to the ground points to the target's location.
Participants are thought to sample from a normal distribution around the true gaze vector.
Individuals would differ in how wide their distribution around the true gaze vector is.
The wider the distribution around the true gaze vector, the more uncertain participants are in estimating the agent's focus of attention.
The so-called inferential component in our model describes this distribution width and represents how accurately the participant infers the target location based on the agent's pupil location.
...
We describe this as a "headlight distribution": when the agent directs their gaze toward the very left or right side, the distribution around the gaze vector from which participants sample is comparatively wider than when the agent gazes centrally to the ground in front of them (Imagine a similar phenomenon: pointing a torch light to the ground. When one directs the light straight onto the ground, the light beam is clearly defined and smaller than when one points the torch light further away from oneself).
...
If our data matches the pattern of this model prediction, this speaks for the gaze model.

However, participants who are overall very imprecise in locating the target might be less likely to use the agent's gaze as a cue at all.
To accommodate this, we designed two models that represent alternatives about how participants solve the TANGO, ignoring the agent's gaze.
One alternative model estimates how likely a participant engages in random guessing.
This acts like a coin toss, as sampling from a uniform distribution over all possible coordinates.
The other alternative model explores the possibility of a center bias: it estimates how likely a participant shows a bias toward the center of the screen.
Partipants' clicks could be drawn toward the center since the agent and the starting point of the balloon are located here.

With our modeling framework, we can formalize whether a participant (A) uses the available gaze information at all and (B) how accurately they are in using this gaze information.
Regarding developmental change, we hypothesize that a dual process could explain the development of gaze understanding: The older children are, the more likely they are to use gaze as a cue, and the more precise they become at it.
...
We compare the three models via BayesFactors computed based on the marginal log-likelihood of the data under each model.
The models make different predictions about how participants' clicks will be distributed for different target locations.
...
Both the model predictions and the observed data show that precision levels drop as the agent's gaze moves further away from the center.
